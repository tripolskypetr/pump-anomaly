...
npm test        # vitest run — 111 тестов
```
...
**Свечи и боковик.** Под каждый кандидат `labelBurst` запрашивает `1m`-свечи вперёд от события на `staleMinutes·2+5` (запас на поздний вход в зону). Запрос идёт одним вызовом `getCandles` — пагинацию на чанки делает твой адаптер, если просит больше лимита биржи. Два защитных контура:
