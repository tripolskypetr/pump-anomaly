...
describe("PUMP — честный памп вверх (детерминированный исход)", () => {
  ...
  it("trailing-take: пик +5%, откат до +3% (≥1% откат) → ЧЕСТНАЯ реализация close +3%", () => {
    const cs = C([
    ...
    // прод выходит маркетом по close свечи-триггера: +3%. Пик +5% нереализуем
    // (о нём известно постфактум) — он в r.peak для диагностики.
  ...
  it("резкий откат от пика +8% к +2% → реализуется close +2% (цена пика недостижима)", () => {
    // пик +8%, потом откат к +2% — trailing срабатывает на close свечи отката:
    // выйти «по пику» физически нельзя, пик виден только постфактум
    const cs = C([
      [100, 100.05, 99.95, 100, 1000],
    ...
    expect(r.reason).toBe("trailing-take");
    expect(r.pnl).toBeCloseTo(0.02, 9); // честный close свечи-триггера
    expect(r.peak).toBeCloseTo(0.08, 9); // пик — только диагностика
  });
